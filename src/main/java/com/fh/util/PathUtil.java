...
public class PathUtil
{
    public static String getPicturePath(String pathType, String pathCategory)
    {
        ...
        if ("visit".equals(pathType)) {} else if ("save".equals(pathType)) {
    ...
    {
        String path = (String.valueOf(Thread.currentThread().getContextClassLoader().getResource(""))).replaceAll("file:/", "").replaceAll("%20", " ")
            .trim();
        if (path.indexOf(":") != 1) {
            path = File.separator + path;
        }
