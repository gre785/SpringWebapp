...
public class ImgmsgController
    extends BaseController
{
    private static final String MENU_URL = "imgmsg/list.do";
    ...
    {
        logBefore(logger, "delete Imgmsg");
        if (!Jurisdiction.buttonJurisdiction(MENU_URL, "del")) {
            return;
        }
        PageData pd = getPageData();
        try {
            _imgmsgService.delete(pd);
            out.write("success");
            out.close();
        } catch (Exception e) {
    ...
    public ModelAndView edit()
        throws Exception
    {
        logBefore(logger, "update Imgmsg");
        if (!Jurisdiction.buttonJurisdiction(MENU_URL, "edit")) {
            return null;
        }
        ModelAndView mv = getModelAndView();
        PageData pd = getPageData();
        _imgmsgService.edit(pd);
        mv.addObject("msg", "success");
        mv.setViewName("save_result");
        return mv;
    }
    ...
    public ModelAndView list(Page page)
    {
        ...
        try {
            ...
            page.setPd(pd);
            List<PageData> varList = _imgmsgService.list(page);
    ...
    {
        logBefore(logger, "to new image message page");
        ModelAndView mv = getModelAndView();
        PageData pd = getPageData();
        try {
    ...
    {
        logBefore(logger, "bulk delete Imgmsg");
        ...
        try {
            ...
            if (null != DATA_IDS && !"".equals(DATA_IDS)) {
                ...
                _imgmsgService.deleteAll(ArrayDATA_IDS);
                pd.put("msg", "ok");
            } else {
                pd.put("msg", "no");
            }
    ...
    {
        logBefore(logger, "exprot Imgmsg to excel");
        ...
        try {
            ...
            List<PageData> varOList = _imgmsgService.listAll(pd);
